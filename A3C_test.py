...
class ACNet(object):
    def __init__(self,scope,globalAC=None):
        ...
        if scope == GLOBAL_NET_SCOPE:
            ...
            with tf.variable_scope(scope):
                ...
                clipped_a_prob = tf.clip_by_value(self.a_prob, 1e-20, 1.0)

                log_prob = tf.reduce_sum(tf.log(clipped_a_prob) * tf.one_hot(self.a_his, N_A, dtype=tf.float32), axis=1,keep_dims=True)
                exp_v = log_prob * td_error
                entropy = -tf.reduce_sum(self.a_prob * tf.log(clipped_a_prob), axis=1, keep_dims=True)  # encourage exploration
                ...
                with tf.name_scope('local_grad'):
                    ...
                    self.ac_grads_clipped = []
                    for grad in self.ac_grads:
                        self.ac_grads_clipped.append(tf.clip_by_norm(grad, 40.0))

                    # a_grads_clipped = []
                    # c_grads_clipped = []
                    # for grad in a_grads:
                    #     a_grads_clipped.append(tf.clip_by_norm(grad, 40.0))
                    # for grad in c_grads:
                    #     c_grads_clipped.append(tf.clip_by_norm(grad, 40.0))

                with tf.name_scope('sync'):
                    with tf.name_scope('pull'):
                        self.pull_ac_params_op = [l_p.assign(g_p) for l_p, g_p in zip(self.ac_params, globalAC.ac_params)]
                        #self.pull_a_params_op = [l_p.assign(g_p) for l_p, g_p in zip(self.a_params, globalAC.a_params)]
                        #self.pull_c_params_op = [l_p.assign(g_p) for l_p, g_p in zip(self.c_params, globalAC.c_params)]
                    with tf.name_scope('push'):
                        #self.update_a_op = optimizer_a.apply_gradients(zip(a_grads_clipped, globalAC.a_params))
                        #self.update_c_op = optimizer_c.apply_gradients(zip(c_grads_clipped, globalAC.c_params))
                        self.update_ac_op = optimizer.apply_gradients(zip(self.ac_grads_clipped, globalAC.ac_params))
                        #pass
    ...
    def update_global(self, feed_dict):  # run by a local
        ...
        #print SESS.run(self.total_loss, feed_dict)
        #print SESS.run(self.a_loss, feed_dict)
        #print SESS.run(self.c_loss, feed_dict)
    ...
    def work(self):
        ...
        while not stop_requested and GLOBAL_EP < MAX_GLOBAL_EP:
            ...
            while not done:
                ...
                s_ = self.process_image(x_t1_colored)
